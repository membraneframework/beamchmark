defmodule Beamchmark do
  ...
  * context switches - total context switches number
  ...
  @default_memory_interval_ms 1000
  @default_delay_s 0
  ...
  * `memory_interval` - time in milliseconds `#{inspect(__MODULE__)}` will be benchmarking memory usage. Defaults to `#{@default_memory_interval_ms}` milliseconds. Needs to be greater than or equal to `interfere_timeout`.
  ...
  * `output_dir` - directory where results of benchmarking will be saved. Defaults to "`beamchmark`" directory under location provided by `System.tmp_dir!/0`.
  ...
  def run(scenario, opts \\ []) do
    config = %Beamchmark.Suite.Configuration{
      ...
      memory_interval: Keyword.get(opts, :memory_interval, @default_memory_interval_ms),
